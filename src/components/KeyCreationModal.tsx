...
import { X, Sparkles, Cpu, Radio, Shield, Copy, Check, Terminal } from 'lucide-react';
...
export const KeyCreationModal: React.FC<KeyCreationModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const generated = onGenerateKey(
        selectedTier,
        selectedRelay,
        name.trim() || `Unit-${selectedTier.split('-')[0].toUpperCase()}`
      );
      setNewKey(generated);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white tracking-tight">
                Request Martian API Key
              </h2>
              <p className="text-xs text-slate-400 font-mono">
                Provision instant credentials on Ares Gateway
              </p>
            </div>
          </div>
            ...
            <div className="space-y-4 animate-in fade-in duration-200">
              <div className="p-4 rounded-2xl bg-emerald-950/20 border border-emerald-500/30 space-y-2">
                ...
                <p className="text-xs text-slate-300">
                  Save your secret key now. It is configured for {newKey.rpmLimit} requests/min on the {newKey.relayZone} relay.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                <button
                  onClick={resetModal}
                  className="flex-1 py-3 rounded-full bg-white hover:bg-slate-200 text-neutral-900 font-bold text-xs transition-colors cursor-pointer"
                >
                  Done
                </button>
                <button
                  ...
                  className="py-3 px-4 rounded-full bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 text-emerald-300 font-mono text-xs transition-colors flex items-center gap-1.5"
                >
                  <Terminal className="w-3.5 h-3.5" />
                  <span>View in CLI</span>
                </button>
              </div>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="e.g., Perseverance Geological Spectrometer"
                  className="w-full bg-[#05080f] border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-amber-500/40 font-mono"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-[#05080f] border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-slate-100 focus:outline-none focus:border-amber-500/40 font-mono"
                >
                  {Object.entries(MODEL_TIER_CONFIG).map(([key, config]) => (
                    <option key={key} value={key} className="bg-neutral-900 text-white">
                      {config.label} ({config.pricing})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-mono text-slate-300 flex items-center gap-1.5">
                  ...
                  <span>Mars Relay Station</span>
                </label>
                <select
                  value={selectedRelay}
                  onChange={(e) => setSelectedRelay(e.target.value as RelayZone)}
                  className="w-full bg-[#05080f] border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-slate-100 focus:outline-none focus:border-amber-500/40 font-mono"
                >
                  {Object.entries(RELAY_ZONES).map(([key, config]) => (
                    <option key={key} value={key} className="bg-neutral-900 text-white">
                      {config.name} (Latency: {config.latency})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <button
                ...
                className="w-full mt-3 py-3.5 rounded-full bg-white hover:bg-slate-200 text-neutral-900 font-bold text-sm transition-all shadow-[0_0_20px_rgba(255,255,255,0.25)] flex items-center justify-center gap-2 cursor-pointer"
              >
                {isSubmitting ? (
